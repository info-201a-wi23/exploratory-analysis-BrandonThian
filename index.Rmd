...
### Table
For H1 visa statistics, the most important information will be the approval/denial trend and the approval rate by year. This is a table that calculates the total sum of approvals and denials by year and the approval rate.

As shown in the statistics table below, the H1 approval rate continued to increase after the COVID-19 pandemic.

```{r, echo = FALSE, code = readLines("table.R"), results='asis'}
```
...
```{r, echo = FALSE, fig.width = 10, code = readLines("chart1.R")}
```


### Chart 2
This chart represents the total number of H1 visa sponsors by industry sector from 2018 to 2022. The x-axis represents the total number of employers(sponsors), and the y-axis represents the industry sector of the sponsors. For more information about the industry sector, please visit https://www.census.gov/naics/
...
```{r, echo = FALSE, fig.width = 10, fig.height = 6, code = readLines("chart2.R")}
```
...
```{r, echo = FALSE, fig.width = 10, code = readLines("chart3.R")}
```
